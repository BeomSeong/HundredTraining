        ...
        <tbody class="table-group-divider">
   		    <c:forEach items="${posts}" var="dto" >
		        <tr>
		            ...
		            <td>${dto.mNick}</td>
		            <td>${dto.cContent}</td>
		            <td><fmt:formatDate value="${dto.cDate}" pattern="yyyy. M. d. a h:mm:ss"/></td>
       				<td>
       					<form id="commentDeleteForm_${dto.cId}">
							<input type="hidden" id="cId" name="cId" value="${dto.cId}">
							<button type="button" name="commentDeleteBtn" id="commentDeleteBtn" class="btn btn-outline-primary" onclick="submitDelComment_ajax('${dto.cId}');">삭제</button>
						</form>
					</td>
		        </tr>
		    </c:forEach>
